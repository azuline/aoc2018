...
parseDeps line = (from, to)
  where
    [from, to] = take 2 $ List.filter (`Set.member` upperCase) $ drop 1 line
...
constructDepsMap deps = depsMap
  where
    withTosInserted = mapTosToFroms deps Map.empty
...
mapTosToFroms ((from, to) : deps) map = mapTosToFroms deps $ Map.alter alterF to map
  where
    alterF (Just froms) = Just (from : froms)
...
part1 map
  ...
  where
    depsWithNoDependencies = List.map fst $ List.filter (null . snd) $ Map.toList map
...
emulateWorkers workers map
  ...
  where
    finishedWorkers = List.filter ((== 0) . snd) workers
...
removeFinishedFromMap map chars = (removeFromValues . stripKeys) map
  where
    removeFromValues = Map.map (List.\\ chars)
